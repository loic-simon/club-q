...
def exporter_fiches_eleves():
    ...
    if lien_dossier:        # Dossier sélectionné (pas Annuler)
        with config.ContextPopup(config.root, "Exportation...") as popup:
            for i_doc, client in enumerate(config.clients):
                pdf_client(client, lien_dossier, open_doc=False, i_doc=i_doc, N_docs=len(config.clients), popup=popup)
...
def exporter_fiches_spectacles():
    ...
    if lien_dossier:        # Dossier sélectionné (pas Annuler)
        with config.ContextPopup(config.root, "Exportation...") as popup:
            for i_doc, spec in enumerate(config.spectacles):
                pdf_spectacle(spec, lien_dossier, open_doc=False, i_doc=i_doc, N_docs=len(config.spectacles), popup=popup)
...
def exporter_excels_spectacles():
    lien_dossier = tk.filedialog.askdirectory(title="Sélectionnez le dossier où exporter les Excel récapitulatifs spectacles :")
    if lien_dossier:        # Dossier sélectionné (pas Annuler)
        with config.ContextPopup(config.root, "Exportation...") as popup:
            for i_doc, spec in enumerate(config.spectacles):
                excel_spectacle(spec, lien_dossier, open_doc=False, i_doc=i_doc, N_docs=len(config.spectacles), popup=popup)

        if tk.messagebox.askyesno(title="Exportation", message=f"Excels spectacles exportées dans {lien_dossier} !\n\nL'ouvrir dans l'explorateur de fichiers ?"):
            open_file(lien_dossier)
...
def pdf_client(client, lien_dossier=None, open_doc=True, i_doc=None, N_docs=None, popup=None):
    ...
    if lien_dossier:    # pas Annuler
        ...
        with config.ContextPopup(config.root, f"Génération du PDF{etape}", existing=popup):
            ...
            if open_doc and tk.messagebox.askyesno(title="Exportation", message=f"Fiche élève individuelle exportée !\n\nL'ouvrir ?"):
                open_file(filepath)
...
def pdf_spectacle(spec, lien_dossier=None, open_doc=True, i_doc=None, N_docs=None, popup=None):
    ...
    if lien_dossier:    # pas Annuler
        ...
        with config.ContextPopup(config.root, f"Génération du PDF{etape}", existing=popup):
            ...
            if open_doc and tk.messagebox.askyesno(title="Exportation", message=f"Fiche spectacle exportée !\n\nL'ouvrir ?"):
                open_file(filepath)
...
def excel_spectacle(spec, lien_dossier=None, open_doc=True, i_doc=None, N_docs=None, popup=None):
    if not lien_dossier:
        lien_dossier = tk.filedialog.askdirectory(title=f"Sélectionnez un dossier pour exporter l'Excel récapitulatif pour {spec.nom} :")

    if lien_dossier:    # pas Annuler

        filepath = os.path.join(os.path.normpath(lien_dossier), f"{filenamify(spec.nom)}.xlsx")

        etape = f" {i_doc+1}/{N_docs}" if N_docs else "..."
        with config.ContextPopup(config.root, f"Génération de l'Excel{etape}", existing=popup):
            ...
            sheet["E4"] = sum((voeu.places_attribuees or 0) for voeu in spec.attributions())
            ...
            npa = 0
            tot = 0
            for i, voeu in enumerate(sorted(spec.attributions(), key=lambda v: v.client.nomprenom)):
                client = voeu.client
    ...
    if open_doc and tk.messagebox.askyesno(title="Fichier exporté", message="Excel récapitulatif exporté ! L'ouvrir ?"):
        open_file(filepath)
