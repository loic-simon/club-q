...
class Eleve:
    ...
    def ajoutervoeu(self, ligne):
        ...
        if (ligne[colnbdeplaces] and etrenombre(ligne[colnbdeplaces]) and etrenombre(ligne[colordre])):
            ...
            if not ligne[colnbmini]:      # si le nbmini est vide, on y met le nb de places demandees
    ...
    def imprimervoeux(self):
        """Imprime la liste de voeux de l'élève"""
...
listespectacles = []
listeeleves = []
liste_attributions_globale = ListeAttributions()
num_voeux = 1
...
label = Label(fenetre_principale, text="Liste des élèves")  # cree un autre bloc contenant une liste
label.pack()
...
def pdf_spectacles():
    global lien_logo
    global lien_logo_q
    # Attention ! renomer les importations de Frame en Frame et Canvas en  Canvas

    lien_pdf=askdirectory(title="Sélectionnez un dossier où les fiches spectacles seront exportées :")
    if lien_pdf != "":


        for spec in listespectacles:

            can = Canvas(f"{lien_pdf}/{spec.titre}.pdf", pagesize=A4)
            ...
            cH =Frame(1*cm, 23.5*cm, 19*cm, 3.5*cm, showBoundary =1)
            cH.addFromList(description, can)

            hauteur = 21.5
            ordonnee = 22.5-hauteur
            cb =Frame(1*cm, ordonnee*cm, 19*cm, hauteur*cm, showBoundary =1)
...
def pdf_eleve(eleve, lien_dossier):
    ...
    if lien_dossier:
        ...
        fichname = f"{lien_dossier}{eleve.nom}_{eleve.prenom}.pdf"
        showerror("oh", f"fichname = {fichname}, type {type(fichname)}")
        can = Canvas(fichname, pagesize=A4)
        styles = getSampleStyleSheet()
        styleN = styles["Normal"]
        can.setFont("Times-Bold", 18)
        ...
        texte[0] += f"<b>{eleve.nom}</b>"
        texte[1] += f"<b>{eleve.prenom}</b>"
        if not eleve.autre:
            texte[2] += f"<b>{eleve.promo}</b>"
        else:
            texte[2] += f"<b>{eleve.autre}</b>"
        texte[3] += f"<b>{eleve.numero_tel}</b>"
        ...
        cH = Frame(1*cm, 23.5*cm, 19*cm, 3*cm, showBoundary =1)
        cH.addFromList(description, can)
...
def exporter_fiches_eleves():
    lien_pdf=askdirectory(title="Sélectionnez un dossier pour exporter les fiches PDF individuelles :")
    if lien_pdf != "":
...
def modif_lien_fiches(label):
    ...
    filepath=askdirectory(title="Répertoire d'accès aux fiches élèves XLSX. Ce répertoire doit contenir UNIQUEMENT les fiches des élèves.")
    if filepath != "":
...
def fiche_eleve(event):
    ...
    Button(infos, text="Imprimer fiche", command=lambda: pdf_eleve(eleve, askdirectory(title="Sélectionnez un dossier pour exporter la fiche PDF de " + eleve.prenom + " :"))).pack(side="right", padx=10)
...
def nouvel_eleve():
    ...
    def valider():
        nval, pval, numval, remval = n.get(), p.get(), num.get(), rem.get()
        if nval and pval and numval and remval:
            listeeleves.append(Eleve(nval, pval, "0", numval, autre=remval))
            update_listes()
            nv.destroy()
        else:
    ...
    Button(nv, text="Valider", command=valider).grid(row=4, column=1, padx=2, pady=2)
...
menusauv.add_separator()
menusauv.add_command(label="Élèves sans places", command=eleves_sans_rien)
...
fenetre_principale.geometry(f"{400}x{550}")
#----------------------------------------------------------------------------------
